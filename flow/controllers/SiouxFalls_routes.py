...
def parse_od_matrix(od_text: str):
    """Parse the 'Origin i ... j : value;' style text into a list-of-lists."""
    rows = []
    current_row = []

    for line in od_text.splitlines():
        # New origin => push previous row (if any) and start a new one
        ...
        # Extract all "index : value" pairs on this line
        for _idx, val in re.findall(r"(\d+)\s*:\s*([0-9.]+)", line):
...
# 24 × 24 matrix; OD[i][j] is trips from origin i+1 to dest j+1
OD_SIOUX_FALLS = parse_od_matrix(OD_RAW)
# Optional: print the matrix as a NumPy array
# print(np.array(OD_SIOUX_FALLS))

# Build function to apply origin weights
def apply_origin_weights(od_matrix, weight_vec):
    """
    Apply origin-based scaling weights to an OD matrix.

    Parameters
    ----------
    od_matrix : np.ndarray or list of lists (shape NxN)
    weight_vec : list or array of length N
        Each entry w_i scales all outbound demand from origin i.

    Returns
    -------
    np.ndarray : weighted OD matrix
    """
    od = np.array(od_matrix, dtype=float)
    w = np.array(weight_vec, dtype=float)

    assert od.shape[0] == od.shape[1], "OD matrix must be square."
    assert len(w) == od.shape[0], "Weight vector length must equal # origins."

    # Create diagonal matrix and multiply
    diagW = np.diag(w)
    weighted = diagW @ od     # scales rows

    return weighted

# Build function to convert OD matrix to {origin: {dest:rate}}
def build_sioux_falls_od_rates(od_matrix, scale=1.0, weights=None):
    """
    Convert OD matrix into {origin: {dest: veh/hour}},
    optionally applying a vector of origin weights (for WCE control).

    weights : None (default) → identity (no change)
              list/array of len N → multiply each origin row
    """
    od = np.array(od_matrix, dtype=float)

    # If no weights given, use all-ones (identity scaling)
    if weights is None:
        weights = np.ones(od.shape[0])

    # Apply diagonal scaling
    weighted_od = apply_origin_weights(od, weights)

    od_rates = {}
    n = weighted_od.shape[0]

    for o_idx in range(n):
        origin_id = str(o_idx + 1)
        od_rates[origin_id] = {}

        for d_idx, trips in enumerate(weighted_od[o_idx]):
            if trips <= 0:
                continue

            dest_id = str(d_idx + 1)
            od_rates[origin_id][dest_id] = float(trips) * float(scale)
...
# ------------------------------------------------------------------------
# ---------Uncomment below to plot the OD matrix as a text table----------
# ------------------------------------------------------------------------

# OD_array = np.array(OD_SIOUX_FALLS)

# # Optional sanity checks
# assert OD_array.shape == (24, 24)

# # ---- text-only "matrix" plot ----
# fig, ax = plt.subplots(figsize=(12, 8))

# # Turn off normal axes
# ax.axis("off")

# # Row/column labels starting at 1 instead of 0
# row_labels = np.arange(1, 25)
# col_labels = np.arange(1, 25)

# # Create a table: each cell shows the OD value
# table = ax.table(
#     cellText=OD_array.astype(int),  # or OD_array if you want decimals
...
# fig.suptitle("Sioux Falls OD matrix", fontsize=14)
# plt.savefig("sioux_falls_od_matrix_table.png", dpi=300, bbox_inches="tight")
# print("Saved OD matrix table to sioux_falls_od_matrix_table.png")
