...
class MultiTrafficLightGridPOEnv(TrafficLightGridPOEnv, MultiEnv):
    ...
    def __init__(self, env_params, sim_params, network, simulator='traci'):
        ...
        if not hasattr(self, "_printed_src_once"):
            print(f"[ENV LOADED] {self.__class__.__name__} from {__file__}")
            print(f"[ENV PARAMS] horizon={self.env_params.horizon}, sim_step={self.sim_params.sim_step}, "
                f"render={getattr(self.sim_params, 'render', None)}, restart_instance={getattr(self.sim_params, 'restart_instance', None)}")
        self._printed_src_once = True
    ...
    def step(self, rl_actions):
        ...
        if done:
            try:
                tc = getattr(self, "time_counter", None)
                nveh = len(self.k.vehicle.get_ids()) if hasattr(self.k, "vehicle") else None
                # SUMO's "expected vehicles left" (running + waiting to depart)
                min_expected = None
                if hasattr(self.k, "kernel_api") and hasattr(self.k.kernel_api, "simulation"):
                    min_expected = self.k.kernel_api.simulation.getMinExpectedNumber()

                reason = []
                if tc is not None:
                    reason.append(f"t={tc}/{self.env_params.horizon}")
                if nveh is not None:
                    reason.append(f"nveh={nveh}")
                if min_expected is not None:
                    reason.append(f"minExpected={min_expected}")

                print("[ENV DONE] " + " ".join(reason))
            except Exception:
                pass
    ...
    def reset(self):
        ...
        if not getattr(self, "_printed_hz_once", False):
            print(f"[ENV] horizon={self.env_params.horizon}, sim_step={self.sim_params.sim_step}, "
                f"restart_instance={getattr(self.sim_params, 'restart_instance', None)}")
            self._printed_hz_once = True

        tc = getattr(self, "time_counter", None)
        print(f"[ENV RESET] new episode; horizon={self.env_params.horizon}, time_counter={tc}")
        # # Print the observation (debug)
        # print("\n[DEBUG] Observation info:")
        # if isinstance(obs, dict):
        #     for agent, ob in obs.items():
        #         print(f"Agent: {agent}, Obs shape: {np.shape(ob)}, Sample: {ob[:10]}")
        # else:
        #     print("Obs shape:", np.shape(obs))
        #     print("First few elements:", obs[:10])
            ...
            for idx in neighbors:
                if idx == -1:
                    ...
                    tl_states.extend([
                        ...
                        self.last_phase[idx] / 4.0,                    # phase normalized to [0,1]
    ...
    def _apply_rl_actions(self, rl_actions):
        ...
        for rl_id, rl_action in rl_actions.items():
            ...
            grp = self._classify_links_for_node(node_id)
            # print(node_id, "L=", grp["L"], 
            #       "N/S/E/W:", [len(grp["north_all"]), len(grp["south_all"]), len(grp["east_all"]), len(grp["west_all"])],
            #       "NS_SR/EW_SR:", [len(grp["ns_sr_lane1"]), len(grp["ew_sr_lane1"])],
            #       "NS_L/EW_L:", [len(grp["ns_left_lane0"]), len(grp["ew_left_lane0"])])
            ...
            # graceful fallbacks for missing groups
            if a == 2 and len(grp["ns_left_lane0"]) == 0:
                a = 0  # no NS left lane -> fall back to NS SR
            if a == 3 and len(grp["ew_left_lane0"]) == 0:
                a = 1  # no EW left lane -> fall back to EW SR
            if a == 4 and len(grp["north_all"]) == 0:
                a = 0  # no North links -> fall back to NS SR
            if a == 5 and len(grp["south_all"]) == 0:
                a = 0  # no South links -> fall back to NS SR
            if a == 6 and len(grp["east_all"]) == 0:
                a = 1  # no East  links -> fall back to EW SR
            if a == 7 and len(grp["west_all"]) == 0:
                a = 1  # no West  links -> fall back to EW SR
